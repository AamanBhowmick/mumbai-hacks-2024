...
def load_image_from_path(img_path):
    img = tf.io.read_file(img_path)  # Read the image from the file path
    img = tf.io.decode_jpeg(img, channels=3)  # Decode the image (3 channels for RGB)
    
    # Resize the image to the input size of InceptionV3
    img = tf.image.resize(img, (299, 299))  
    
    # Cast the image to float32
    img = tf.cast(img, tf.float32)
    
    # Preprocess the image using InceptionV3's preprocess function
